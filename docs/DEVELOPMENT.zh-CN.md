...
低于清晰阈值时，把屏幕视为固定在阈值角度上，而盖子在真实移动，观察者正对这块参考屏幕看，眼睛与屏幕顶边齐平、距离 5 个屏幕高度（`EffectModel.eyeDistance`）。盖子朝眼睛倾斜了 x − y，顶边在眼睛看来比参考屏幕的更近更大，因此内容顶部要画窄 tan(x − y) / eyeDistance 来补偿：相差 30° 时宽 88%，60° 时 65%。相对盖子，内容向后倾倒了 x − y，所以顶部还按透视缩短 cos(x − y) 往下落，幅度由 `EffectModel.verticalShrink` 控制：相差 30° 时高 92%，60° 时 70%，上方留空。完整投影则会把内容拉伸到顶边之外，这部分被刻意去掉，画面始终留在屏幕内。只有两个平面之间的夹角起作用。「透视强度」滑块控制几何应用多少，0% 画面不变形，100% 完整收窄和缩短。最大暗化为 65%，高斯模糊强度可调。Retina 原始分辨率捕获，60 帧/秒，Core Image 直接写入 Metal 显示纹理；透视、暗化与模糊按时间统一平滑，仅在效果生效时运行捕获。设置窗口保持可读；效果层不拦截鼠标。
